...
export default function PresidentDashboard() {
  ...
  async function fetchPresidentData() {
    ...
    // 1. Récupérer le profil du président
    ...
    // 2. Récupérer Candidats + TOUTES les notes (avec jointure pour les noms des jurys)
    const [candRes, evalRes] = await Promise.all([
      ...
      supabase.from('evaluations')
        .select(`
          *,
          jury:profiles!jury_id(full_name)
        `)
    ]);

    // Filtrage pour ne garder que les notes liées aux candidats de la catégorie du président
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <button 
                ...
                className={`p-1 transition-all ${refreshing ? 'animate-spin' : 'hover:rotate-180'}`}
              >
      ...
      <div className="max-w-6xl mx-auto px-6 mt-10">
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
          <div className="bg-white p-5 rounded-[2rem] border border-gray-100 shadow-sm">
            <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">Candidats</p>
            <p className="text-2xl font-black text-gray-900">{candidats.length}</p>
          </div>
          <div className="bg-white p-5 rounded-[2rem] border border-gray-100 shadow-sm">
            <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">Total Notes</p>
            <p className="text-2xl font-black text-indigo-600">{allEvaluations.length}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8">
          {candidats.map((c) => {
            ...
            const allNotesForThisCandidat = allEvaluations.filter(e => e.candidat_id === c.id);
            const moyenneGénérale = allNotesForThisCandidat.length > 0 
              ? allNotesForThisCandidat.reduce((acc, curr) => acc + curr.total_score, 0) / allNotesForThisCandidat.length 
              : 0;
            ...
            return (
              <div key={c.id} className="bg-white rounded-[3rem] border border-gray-100 shadow-sm overflow-hidden transition-all hover:shadow-xl hover:shadow-indigo-100/50">
                <div className="p-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
                  ...
                  {/* MILIEU : SUPERVISION DÉTAILLÉE DES EXPERTS */}
                    ...
                    {othersNotes.length > 0 ? (
                      othersNotes.map((note, idx) => (
                        <div key={idx} className="group relative flex flex-col items-center">
                          <div className="h-12 w-12 rounded-2xl bg-white border border-gray-200 flex items-center justify-center shadow-sm group-hover:border-indigo-400 transition-all cursor-help">
                            <span className="text-xs font-black text-gray-800">{note.total_score}</span>
                          </div>
                          <span className="mt-1 text-[7px] font-bold text-gray-400 uppercase truncate max-w-[50px]">
                            {note.jury?.full_name?.split(' ')[0] || "Expert"}
                          </span>
                          {/* Tooltip */}
                          <div className="absolute -top-10 scale-0 group-hover:scale-100 transition-all bg-gray-900 text-white text-[8px] px-2 py-1 rounded-lg z-50 whitespace-nowrap">
                            {note.jury?.full_name}
                          </div>
                        </div>
                      ))
                    ) : (
                      <p className="text-[9px] font-bold text-gray-300 uppercase italic px-4">Aucune note expert reçue</p>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-3 w-full lg:w-auto">
                    {c.document_url && (
                      <a href={c.document_url} target="_blank" rel="noreferrer" className="p-4 bg-gray-900 text-amber-400 rounded-2xl hover:bg-gray-800 transition-all">
                    ...
                    <Link to={`/jury/notation/${c.id}`} className="flex-1 lg:flex-none">
                      <button className={`w-full px-10 py-5 rounded-[2rem] text-[11px] font-black uppercase tracking-widest transition-all ${
                        ...
                        {myNote ? 'Réviser mon vote' : 'Noter le candidat'}
                      </button>
                    </Link>
                  </div>
                </div>

                {/* FOOTER : MOYENNE D'ARBITRAGE */}
                <div className="bg-indigo-50/50 px-8 py-5 flex flex-col sm:flex-row items-center justify-between border-t border-indigo-50 gap-4">
                  ...
                  <div className="flex items-center gap-6 w-full sm:w-auto">
                    <div className="flex-1 sm:w-48 h-3 bg-white rounded-full overflow-hidden border border-indigo-100 p-[2px]">
                      <div 
                        className="h-full bg-gradient-to-r from-indigo-500 to-amber-400 rounded-full transition-all duration-1000" 
                        style={{ width: `${(moyenneGénérale / 30) * 100}%` }}
                      ></div>
                    </div>
                    <div className="flex items-baseline gap-1 bg-white px-4 py-1 rounded-xl border border-indigo-100 shadow-sm">
                      <span className="text-xl font-black text-indigo-600">{moyenneGénérale.toFixed(2)}</span>
        ...
        {candidats.length === 0 && (
          <div className="text-center py-32 bg-white rounded-[4rem] border-4 border-dashed border-gray-100 mt-10">
            <div className="bg-gray-50 h-20 w-20 rounded-full flex items-center justify-center mx-auto mb-4">
              <UserGroupIcon className="h-10 w-10 text-gray-200" />
            </div>
            <h3 className="text-gray-400 font-black uppercase tracking-widest">Aucun candidat détecté</h3>
            <p className="text-gray-300 text-xs mt-2">Vérifiez la configuration de la catégorie dans l'admin.</p>
          </div>
        )}
      </div>
    </div>
